...
chickPlot + geom_histogram(aes(x=weight, y=..density..), binwidth =7) + 
...
```{r}
diamonds.subset = subset(x = diamonds, subset = (diamonds$cut == "Good" | diamonds$cut == "Fair"))
```
...
ggplot(diamonds.subset) +
  geom_histogram(mapping=aes(x=price, y=..density..,fill=cut, alpha = 0.5)) +
  labs(x = "Price in US dollars",
       y = "Proportions of diamond per dollar",
       title = "Round cut diamonds")
```
...
```{r}
ggplot(diamonds.subset) +
  geom_histogram(mapping=aes(x=price, y=..density..,fill=as.factor(cut), alpha = 0.5)) +
  facet_grid(as.factor(cut)) +
  labs(x = "Price in US dollars",
       y = "Proportions of diamond per dollar",
       title = "Round cut diamonds")
```
